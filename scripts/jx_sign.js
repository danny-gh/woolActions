...
const WQ_API_HOST = 'https://wq.jd.com/';
async function jx_fanxiantask() {
  await signhb_query();
  if($.commontask.length > 0){
    await signhb_commontask();
  }
}

function signhb_query() {
  return new Promise((resolve) => {
    //$.get(fxtaskUrl("fanxiantask/signhb/query", "signhb_source=5"), async (err, resp, data) => {
    $.get(fxtaskUrl("fanxiantask/signhb/query"), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          //console.log(`return from fanxiantask/signhb/query:\n${data}`)
          let jsonData = data.replace("try{Query(", "").replace(");}catch(e){}", "")
          if (safeGet(jsonData)) {
            data = JSON.parse(jsonData);
            console.log(`${$.UserName}的share_qywx: ${data.share_qywx}`)
            let shareCode={
              "smp": data.smp,
              "share_qywx": data.share_qywx,
            }
            $.newShareCodes.push(shareCode);
            $.commontask = data.commontask;
            //$.sharetask = data.sharetask;
          }else{
            console.log(`safeGet failed:\n${data}`)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
...
async function signhb_commontask() {
  for (task of $.commontask) {
    if (task.status === 1) {
      console.log(`去做${task.taskname}任务`)
      await signhb_dotask(task.task);
      await $.wait(1000)
    }
  }
}

function signhb_dotask(task) {
  return new Promise((resolve) => {
    $.get(fxtaskUrl("fanxiantask/signhb/dotask", `task=${task}`), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          data = data.replace("try{Dotask(", "").replace(");}catch(e){}", "");
          console.log(`signhb_dotask returns:\n${data}`)
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
function fxtaskUrl(functionId, body = '') {
  return {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            console.log(data);
            data = JSON.parse(data);
            if(data.retCode ===0 && data.data.tasks != null){

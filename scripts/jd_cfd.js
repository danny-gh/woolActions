      ...
      try {
        if (err) {
          ...
          if (showInvite && strMyShareId) {
            ...
            $.newShareCodes.push(strMyShareId);
          }
          $.info = {
            ...$.info,
            SceneList,
    ...
    switch (taskType) {
      case 0://日常任务
        ...
        for (let i = 0; i < $.allTask.length; i++) {
          ...
          for (let j = taskinfo.completedTimes; j < taskinfo.targetTimes; j++) {
            await $.wait(3000);
            if (status[0]) {
              //做任务
              status[0] = await doTask(taskinfo, j);
            }
            await $.wait(3000);
            if (status[1]) {
              //领取奖励
...
//做任务
function doTask(taskinfo, taskTimes = 0) {
  return new Promise(async (resolve) => {
    const { taskId, completedTimes, configTargetTimes, taskName, taskConfigExtra} = taskinfo;
    ...
    let url = '';
    if(parseInt(taskId) == 1592){
      const cfgext = JSON.parse(taskConfigExtra);
      const configExtraList = cfgext.shopid.split`,`;
      const configExtra = configExtraList[taskTimes];
      console.log(`configExtra: ${configExtra}`)
      url = taskListUrl(`DoTask`, `taskId=${taskId}&configExtra=${configExtra}`);
    }else{
      url = taskListUrl(`DoTask`, `taskId=${taskId}`);
    }

const $ = new Env('hourly_task');
let cookiesArr = [], cookie = '', userName, message;
let isLogin = true;
...
function plantBean_request(function_id, body = {}){
  return new Promise(async resolve => {
    await $.wait(2000);
    $.post(plantBean_taskUrl(function_id, body), (err, resp, data) => {
      try {
        if (err) {
          console.log('\n种豆得豆: API查询请求失败 ‼️‼️')
...
function plantBean_taskUrl(function_id, body) {
